...
class DE
{
	public:
		...
		decltype(newFileMaker("DE")) newFile;
		...
		void Run()
		{
			for(int i = 0; i < LOOP_COUNT; i++)
			{
				...
				if (DIM == 2 || i % 20 == 0) {
					newFile()(R"( { "type": "DE", "data": { "round": %d, "best": %lf } } )", i, bestFitness);
					auto print = newFile();
					print(R"( {"type":"group", "data":[ )");
					for (int i = 0; i < POP_SIZE; i++) {
						print(R"( %lf )", curFitness[i]);
						print(i == N - 1 ? "]}" : ",");
					}
				}
				if (DIM == 2)
...
void DiffEvo(int func_num, int D)
{
	srand((int)time(NULL));
	int count = 20;
	if (D == 2)
